...
void setup(void)
{
  ...
  input_setup();
  sound_init();
  auth_setup(); // Initialize UART communication with Arduino
  scene = AUTH; // Start with authentication screen
}

uint8_t scene = AUTH; // Start with authentication screen
...
// Authentication screen - Password entry and system unlock
void loopAuth()
{
  display.clearRect(1, 1, 127, 63);

  uint32_t authStartTime = millis();
  uint8_t dotCount = 0;
  ...
  while (!exit_scene)
  {
    ...
    {
      display.clearRect(1, 1, 127, 63);
      drawText(SCREEN_WIDTH / 2.5 - 20, SCREEN_HEIGHT * .3, F("ACCESS"));
      drawText(SCREEN_WIDTH / 2.5 - 15, SCREEN_HEIGHT * .45, F("GRANTED"));
      display.display();
    ...
    // Display lock screen with animation while waiting for password
    display.clearRect(1, 1, 127, 63);

    drawText(SCREEN_WIDTH / 2.5 - 20, SCREEN_HEIGHT * .2, F("SYSTEM"));
    drawText(SCREEN_WIDTH / 2.5 - 15, SCREEN_HEIGHT * .35, F("LOCKED"));

    drawText(SCREEN_WIDTH / 2.6 - 25, SCREEN_HEIGHT * .55, F("Enter Password"));

    // Animated dots to show waiting
    dotCount = ((millis() - authStartTime) / 500) % 4;
    display.clearRect(SCREEN_WIDTH / 2.2 - 20, SCREEN_HEIGHT * .7, 40, 6);
    for (uint8_t i = 0; i < dotCount; i++)
    {
      drawText(SCREEN_WIDTH / 2.2 - 15 + (i * 5), SCREEN_HEIGHT * .7, F("."));
    }

    display.display();

    // Timeout after 60 seconds (optional safety feature)
    if (millis() - authStartTime > 60000)
    {
      // Reset or stay locked
...
void loopGamePlay()
{
  ...
  do
  {
    ...
    // Update authentication state to check for input from Arduino
    auth_update();
